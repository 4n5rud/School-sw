    ...
    const loadCourses = async () => {
      try {
        ...
        if (selectedCategory !== 'ALL') {
          // 카테고리 필터 적용
          response = await courseService.getCoursesByCategory(selectedCategory, 0, PAGE_SIZE);
        } else if (searchKeyword.trim()) {
          // 검색 적용
          response = await courseService.searchCourses(searchKeyword, undefined, 0, PAGE_SIZE);
        } else {
          // 전체 조회
  ...
  useEffect(() => {
    ...
    const loadPage = async () => {
      try {
        ...
        if (selectedCategory !== 'ALL') {
          response = await courseService.getCoursesByCategory(selectedCategory, currentPage, PAGE_SIZE);
        } else if (searchKeyword.trim()) {
          response = await courseService.searchCourses(searchKeyword, undefined, currentPage, PAGE_SIZE);
        } else {
